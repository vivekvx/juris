...
async def _stream(
    ...
    conv = await asyncio.to_thread(get_conversation, conv_id, uid)
    history = await asyncio.to_thread(list_messages, conv_id, uid, 10, True)

    await asyncio.to_thread(create_message, conv_id, uid, content)
    ...
    try:
        ...
        _log.warning("Retrieval failed for conv %s: %s", conv_id, exc)
    ...
    try:
        ...
        _log.error("LLM stream error for conv %s: %s", conv_id, exc)
        final = accumulated + ("\n\n[Response interrupted]" if accumulated else "[Response failed]")
        await asyncio.to_thread(create_assistant_message, conv_id, uid, final, citation_dicts)
        yield _sse("error", {"detail": "Response generation failed. Partial response saved."})
        return

    asst_msg = await asyncio.to_thread(
        create_assistant_message, conv_id, uid, accumulated, citation_dicts
    )
    ...
    if not conv.title_generated:
        asyncio.create_task(_save_title(conv_id, uid, content, accumulated))
...
async def send_message(
    ...
    return StreamingResponse(
        _stream(conv_id, body.content, body.document_ids, current_user.uid),
